...
PARQUET = "/Users/gabrielnixonraj/Desktop/ibl_project/data/ibl_trials_339sessions_with_stim.parquet"
...
def main():
    ...
    for i, eid in enumerate(eids):
        ...
        stim = g["stim_signed_contrast"].to_numpy(dtype=np.float32)
        choice = g["choice"].to_numpy(dtype=np.float32)
        reward = (g["feedbackType"].to_numpy(dtype=np.float32) > 0).astype(np.float32)
